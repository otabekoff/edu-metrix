  ...
  return (
    <SidebarProvider defaultOpen={!sidebarCollapsed} open={!sidebarCollapsed} onOpenChange={(open) => setSidebarCollapsed(!open)}>
      <div className={`
        ...
        min-h-screen w-full flex font-sans overflow-x-hidden relative transition-colors duration-200
      `}>
        ...
        {state.activeRole !== 'Guest' && (
          <Sidebar collapsible="icon" className="border-r border-slate-200/80 dark:border-zinc-800/60 bg-white/90 dark:bg-zinc-900/95 backdrop-blur-2xl">
            {/* Sidebar Header / Logo */}
            <SidebarHeader className="h-16 border-b border-slate-200/80 dark:border-zinc-800/50 flex items-center justify-between px-4 group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:justify-center">
              <div 
                onClick={() => navigate('/')}
                className="flex items-center gap-2.5 group-data-[collapsible=icon]:gap-0 cursor-pointer group"
              >
                <img
                  src={logoUrl}
                  alt="Edumetric"
                  className="w-9 h-9 rounded-lg object-contain shadow-lg shadow-indigo-500/25 dark:shadow-indigo-500/10 group-hover:scale-105 transition-transform duration-200"
                />
                <div className="group-data-[collapsible=icon]:hidden">
                  <span className="font-black text-sm tracking-tight text-slate-800 dark:text-white uppercase bg-linear-to-r from-slate-900 via-slate-800 to-slate-600 dark:from-white dark:via-slate-100 dark:to-slate-300 bg-clip-text">Edumetric</span>
            ...
            <SidebarContent className="px-3 py-4 space-y-6 custom-scrollbar">
              ...
              <SidebarGroup>
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {[
                      { id: 'Student', label: 'Talaba oynasi', icon: GraduationCap, color: 'text-indigo-500 dark:text-indigo-400', path: '/student/overview' },
                      { id: 'Mentor', label: 'Mentor oynasi', icon: BookOpen, color: 'text-purple-500 dark:text-purple-400', path: '/mentor/journal' },
                      { id: 'Admin', label: 'Admin oynasi', icon: Shield, color: 'text-emerald-500 dark:text-emerald-400', path: '/admin/matrix' }
                    ].map(role => {
          ...
          <footer className="relative z-10 w-full py-8 border-t border-slate-200/80 dark:border-zinc-800/40 bg-white/60 dark:bg-zinc-950/60 backdrop-blur-md mt-auto">
            <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400 dark:text-slate-500">
              ...
              <div className="flex items-center gap-1.5 font-bold text-slate-600 dark:text-slate-400">
                Loyihani yaratdi: 
                <span className="bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500 dark:from-indigo-400 dark:via-purple-400 dark:to-pink-400 bg-clip-text text-transparent uppercase tracking-wider font-extrabold animate-pulse">
                  Dominant
                </span> 
                jamoasi
              </div>
