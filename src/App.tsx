...
function AppContent() {
  ...
  return (
    <div className="dark min-h-screen w-full bg-[#030014] text-slate-100 flex font-sans selection:bg-indigo-500/30 selection:text-indigo-200 overflow-x-hidden relative">
      ...
      <div className="fixed bottom-0 right-1/4 w-[500px] h-[500px] bg-purple-500/5 rounded-full blur-[120px] pointer-events-none z-0" />
      ...
      {state.activeRole !== 'Guest' && (
        <>
          ...
          <aside className={`
            ...
            flex flex-col border-r border-slate-800/60 bg-[#09051d]/90 backdrop-blur-2xl
            transition-all duration-300 ease-in-out
            ${sidebarCollapsed ? 'w-20' : 'w-64'}
            ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
          `}>
            {/* Sidebar Header / Logo */}
            <div className="h-16 border-b border-slate-800/50 flex items-center justify-between px-4">
              <div 
                onClick={() => handleRoleChange('Guest')}
                className="flex items-center gap-2.5 cursor-pointer group"
              >
                <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 p-0.5 flex items-center justify-center shadow-lg shadow-indigo-500/25 group-hover:scale-105 transition-transform duration-200">
                  <div className="w-full h-full rounded-[6px] bg-[#030014] flex items-center justify-center">
                    <Sparkles size={14} className="text-indigo-400 group-hover:text-white transition-colors" />
                  </div>
                </div>
                {!sidebarCollapsed && (
                  <div>
                    <span className="font-black text-sm tracking-tight text-white uppercase bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent">Edumetric</span>
                    <span className="text-[9px] block font-extrabold text-indigo-400 tracking-wider">DOMINANT TEAM</span>
                  </div>
                )}
              </div>
              ...
              <button 
                ...
                className="hidden md:flex w-6 h-6 rounded-md bg-slate-900 border border-slate-800 items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-colors"
              >
              ...
              <div className="p-4 border-b border-slate-800/40 bg-slate-950/20">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    />
                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-[#09051d]" />
                  </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
              {/* Main Tab Lists depending on active role */}
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 block mb-2">
                  {sidebarCollapsed ? 'NAV' : 'MENU NAVIGATSIYA'}
                </span>
                <nav className="space-y-1">
                  {/* STUDENT NAVIGATION LINKS */}
                  {state.activeRole === 'Student' && [
                    { id: 'overview', label: 'Boshqaruv Paneli', icon: LayoutDashboard, color: 'text-indigo-400' },
                    { id: 'courses', label: 'Fanlar & Vazifalar', icon: BookOpen, color: 'text-purple-400' },
                    { id: 'achievements', label: 'Yutuqlar Portfolioli', icon: Award, color: 'text-pink-400' },
                    { id: 'scholarship', label: 'Grant Monitoring', icon: Activity, color: 'text-sky-400' },
                    { id: 'feedback', label: 'Mentor Fikrlari', icon: UserCheck, color: 'text-emerald-400' },
                    { id: 'settings', label: 'Sozlamalar', icon: Sliders, color: 'text-amber-400' },
                  ].map(tab => {
                    const Icon = tab.icon;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-200 cursor-pointer ${
                          isActive 
                            ? 'bg-indigo-600/10 text-indigo-200 border border-indigo-500/20 shadow-md shadow-indigo-600/5' 
                  ...
                  {state.activeRole === 'Mentor' && [
                    { id: 'journal', label: 'Jurnal & Davomat', icon: Calendar, color: 'text-purple-400' },
                    { id: 'feedback', label: 'Feedback yuborish', icon: UserCheck, color: 'text-indigo-400' },
                  ].map(tab => {
                    const Icon = tab.icon;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-200 cursor-pointer ${
                          isActive 
                            ? 'bg-purple-600/10 text-purple-200 border border-purple-500/20 shadow-md shadow-purple-600/5' 
                            : 'text-slate-400 hover:text-white hover:bg-slate-900/60 border border-transparent'
                        }`}
                        title={tab.label}
                      >
                  ...
                  {state.activeRole === 'Admin' && [
                    { id: 'matrix', label: '16-Kolonnali Jurnal', icon: LayoutDashboard, color: 'text-emerald-400' },
                    { id: 'queue', label: 'Yutuqlar Navbati', icon: Award, color: 'text-pink-400' },
                    { id: 'modifiers', label: 'Jarima & Bonuslar', icon: Sliders, color: 'text-amber-400' },
                    { id: 'faceid', label: 'FaceID Playground', icon: Terminal, color: 'text-indigo-400' },
                  ].map(tab => {
                    const Icon = tab.icon;
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold transition-all duration-200 cursor-pointer ${
                          isActive 
                            ? 'bg-emerald-600/10 text-emerald-200 border border-emerald-500/20 shadow-md shadow-emerald-600/5' 
                            : 'text-slate-400 hover:text-white hover:bg-slate-900/60 border border-transparent'
                        }`}
                        title={tab.label}
                      >
              ...
              <div>
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-3 block mb-2">
                  {sidebarCollapsed ? 'ROLE' : 'IMPERSONATION (TEZKOR)'}
                </span>
                <nav className="space-y-1">
                  {[
                    { id: 'Student', label: 'Talaba oynasi', icon: GraduationCap, color: 'text-indigo-400' },
                    { id: 'Mentor', label: 'Mentor oynasi', icon: BookOpen, color: 'text-purple-400' },
                    { id: 'Admin', label: 'Admin oynasi', icon: Shield, color: 'text-emerald-400' }
                  ].map(role => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {!sidebarCollapsed && <span className="truncate">{role.label}</span>}
                      </button>
                    );
                  })}
                </nav>
            ...
            <div className="p-4 border-t border-slate-800/50">
              <button
                onClick={() => handleRoleChange('Guest')}
                className={`
                  w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-bold text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition-all cursor-pointer
      ...
      {/* Main Content Area */}
      <div className="flex-1 min-h-screen flex flex-col min-w-0 relative z-10">
        
        {/* Modern low-profile sticky breadcrumbs header */}
        {state.activeRole !== 'Guest' && (
          <header className="sticky top-0 z-40 w-full border-b border-slate-800/50 bg-[#030014]/60 backdrop-blur-xl h-16">
            <div className="h-full px-4 md:px-8 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="md:hidden p-2 text-slate-400 hover:text-white rounded-md hover:bg-slate-900 transition-colors"
                >
                  <Menu size={20} />
                </button>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-slate-500 font-bold uppercase tracking-wider">Edumetric</span>
                  <span className="text-slate-700">/</span>
                  <span className="text-xs font-bold text-indigo-400 tracking-wide bg-indigo-500/5 px-2.5 py-1 rounded border border-indigo-500/10">
                    {getBreadcrumbTitle()}
                  </span>
                </div>
              </div>

              {/* Right Side: Global mock indicators and Avatar Switcher */}
              <div className="flex items-center gap-4">
                {/* Simulated live API Connection */}
                <div className="hidden sm:flex items-center gap-2 bg-[#09051d]/40 border border-slate-800 px-3 py-1.5 rounded-lg text-[10px] text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wider border ${
                    state.activeRole === 'Admin' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                    state.activeRole === 'Mentor' ? 'bg-purple-500/10 border-purple-500/20 text-purple-400' :
                    state.activeRole === 'Student' ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' :
        ...
        <footer className="relative z-10 w-full py-8 border-t border-slate-800/40 bg-slate-950/60 backdrop-blur-md mt-auto">
          <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
            ...
            <div className="flex items-center gap-1.5 font-bold">
              Loyihani yaratdi: 
              <span className="bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent uppercase tracking-wider font-extrabold animate-pulse">
                Dominant
              </span> 
              jamoasi
            </div>
