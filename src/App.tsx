...
function AppContent() {
  ...
  return (
    ...
    `}>
      ...
      <div className="fixed top-0 left-1/4 w-[500px] h-[500px] bg-indigo-500/5 dark:bg-indigo-500/5 rounded-full blur-[120px] pointer-events-none z-0" />
      <div className="fixed bottom-0 right-1/4 w-[500px] h-[500px] bg-purple-500/5 dark:bg-purple-500/5 rounded-full blur-[120px] pointer-events-none z-0" />

      {/* Sidebar Navigation (Fixed on all viewports) */}
      {state.activeRole !== 'Guest' && (
        <>
          ...
          {/* Core Sidebar Shell - FIXED */}
          <aside className={`
            fixed top-0 left-0 h-screen z-50
            flex flex-col border-r border-slate-200/80 dark:border-zinc-800/60 bg-white/90 dark:bg-zinc-900/95 backdrop-blur-2xl
            transition-all duration-300 ease-in-out
            ${sidebarCollapsed ? 'w-20' : 'w-64'}
            ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
          `}>
            {/* Sidebar Header / Logo */}
            <div className="h-16 border-b border-slate-200/80 dark:border-zinc-800/50 flex items-center justify-between px-4">
              <div 
                onClick={() => handleRoleChange('Guest')}
                className="flex items-center gap-2.5 cursor-pointer group"
              >
                <div className="w-9 h-9 rounded-lg bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 p-0.5 flex items-center justify-center shadow-lg shadow-indigo-500/25 dark:shadow-indigo-500/10 group-hover:scale-105 transition-transform duration-200">
              ...
              <button 
                ...
                className="hidden md:flex w-6 h-6 rounded-md bg-slate-100 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 items-center justify-center text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-zinc-900 transition-colors"
              >
            ...
            {!sidebarCollapsed && (
              <div className="p-4 border-b border-slate-200/80 dark:border-zinc-800/40 bg-slate-50/50 dark:bg-zinc-950/20">
            ...
            <div className="p-4 border-t border-slate-200/80 dark:border-zinc-800/50">
              <button
                onClick={() => handleRoleChange('Guest')}
                className={`
                  w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-bold text-rose-500 dark:text-rose-400 hover:text-rose-600 dark:hover:text-rose-300 hover:bg-rose-500/10 transition-all cursor-pointer
      ...
      {/* Main Content Area (Layout padding accounts for fixed sidebar) */}
      <div className={`
        flex-1 min-h-screen flex flex-col min-w-0 relative z-10 transition-all duration-300
        ${state.activeRole === 'Guest' ? '' : sidebarCollapsed ? 'md:pl-20' : 'md:pl-64'}
      `}>
            ...
            <div className="h-full px-4 md:px-8 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="md:hidden p-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white rounded-md hover:bg-slate-100 dark:hover:bg-zinc-900 transition-colors"
                >
                  <Menu size={20} />
                </button>
